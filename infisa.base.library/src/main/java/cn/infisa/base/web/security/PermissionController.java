...
public class PermissionController {
	...
	private static final String URL_BASE = "/security/permission/";
	@Autowired
	private  SecurityService securityService;
	
	@RequestMapping(method=RequestMethod.GET)
	public ModelAndView execute(Model model){
		model.addAttribute("list", securityService.getAllPermissions());
	...
	public ModelAndView save(@Valid Permission permission, RedirectAttributes redirectAttributes){
		securityService.savePermission(permission);
		redirectAttributes.addFlashAttribute("message", "保存权限" + permission.getName() + "成功");
	...
	public ModelAndView update(@PathVariable("id") Long id, Model model){
		model.addAttribute("entity",securityService.getPermission(id));//存入一个实体对象
		return new ModelAndView(URL_BASE+"input");
	}
}
